...
:::tip
[Click here](tutorial-introduction) for an end-to-end **tutorial on how to evaluate an LLM medical chatbot** using `deepeval`.
:::
...
`deepeval` runs evaluations locally on your enviornment. However, you can also **run evaluations directly on** [Confident AI](confident-ai-introduction), an all in one evaluation platform:
...
:::tip
Running evaluations on Confident AI's infrastructure may use Confident AI's proprietary models instead. **[Contact us](https://calendly.com/jeffreyip-myw/confident-ai-intro-call)** if you're dealing with sensitive data that has to reside in your private VPCs.
:::
...
You can view and analyze your **test run results** on Confident AI by logging into the platorm using the following command:

```python
deepeval login
```

Alternatively, you can also save test results locally for each test run. Simply set the `DEEPEVAL_RESULTS_FOLDER` environment variable to your relative path of choice:
...
:::tip Did you know?
You can now run all of `deepeval`'s metrics for free on Confident AI's infrastructure. Start by following [this](confident-ai-testing-n-evaluation-experiments) guide.
:::
...
:::note
A `Golden` is simply an `LLMTestCase` with no `actual_output`, and it is an important concept if you're looking to generate LLM outputs at evaluation time. To learn more about `Golden`s, [click here.](evaluation-datasets#with-goldens)
:::
...
[Visit the synthesizer section](synthesizer-introduction) to learn how to customize `deepeval`'s synthetic data generation capabilities to your needs.
...
`deepeval`'s `RedTeamer` is highly customizable and offers a range of different advanced red teaming capabilities for anyone to leverage. We highly recommend you read more about the `RedTeamer` at the [red teaming section.](red-teaming-introduction)

:::tip
The `TargetLLM()` you see being provided as argument to the `target_model` parameter is of type `DeepEvalBaseLLM`, which is basically a wrapper to wrap your LLM application into `deepeval`'s ecosystem for easy evaluating. You can learn how to create a custom `TargetLLM` in a few lines of code [here.](guides-using-custom-llms#creating-a-custom-llm)
:::
...
:::tip
[Click here](confident-ai-introduction) for the full documentation on using Confident AI through `deepeval`.
:::
...
Confident AI allows anyone to [monitor, trace, and evaluate LLM responses in real-time.](confident-ai-llm-monitoring) A single API request is all that's required, and this would typically happen at your servers right before returning an LLM response to your users:
...
:::info
You can also trace LLM applications on Confident AI. Learn more about how to setup tracing [here.](confident-ai-tracing)
